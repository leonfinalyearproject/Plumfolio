...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (email, password) => {
    try {
      ...
      if (data?.user) { setUser(data.user); await fetchProfileById(data.user.id); }
  ...
  const updateProfile = async (updates) => {
    ...
    try {
      ...
      if (data?.length > 0) { setProfile(data[0]); return { data: data[0], error: null }; }
  ...
  return (
    <AuthContext.Provider value={{ user, profile, loading, signUp, signIn, signOut, updateProfile, updatePassword, fetchProfile }}>
      {children}
