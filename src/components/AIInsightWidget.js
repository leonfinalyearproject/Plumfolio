...
function getPageInsights(pathname, insights, predictions, formatCurrency, crossSignals = []) {
  ...
  // Cross-reference signals — high severity only, max 1
  const compoundItems = (crossSignals || [])
  ...
  switch (page) {
    ...
    case 'budgets': {
      ...
      // Budget warnings (exceeded only on this page — rest on Insights)
      budgetWarnings.filter(w => w.type === 'exceeded').slice(0, 2).forEach(w => {
        items.push({
          type: w.type,
          severity: w.severity,
          message: w.message,
      ...
      // Suggested budget allocations — top 1 only
      budgetSuggestions.slice(0, 1).forEach(s => {
        items.push({
          type: 'suggestion',
          severity: 'info',
          message: s.message,
      ...
      // Category trend — 1 max
      Object.entries(categoryForecasts).slice(0, 1).forEach(([cat, pred]) => {
